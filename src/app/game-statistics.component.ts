...
@Component({
  ...
  template: `
    ...
    <p-button
      ...
      class="theme-shadow-sm hover:theme-shadow-md transition-all duration-200"
    ></p-button>
    ...
    >
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div class="theme-bg-muted rounded-lg p-4 theme-border border">
            <div class="flex items-center justify-between mb-2">
              <span class="text-sm font-medium theme-text-muted">{{
                translationService.translate("statistics.netWorth")
              }}</span>
              <i class="pi pi-wallet text-primary-500"></i>
            </div>
            <p class="text-2xl font-bold theme-text-primary">
              {{ getNetWorth() | number: "1.0-0" }}€
            </p>
          ...
          <div class="theme-bg-muted rounded-lg p-4 theme-border border">
            <div class="flex items-center justify-between mb-2">
              <span class="text-sm font-medium theme-text-muted">{{
                translationService.translate("statistics.totalReturns")
              }}</span>
              <i class="pi pi-chart-line text-success-500"></i>
            </div>
            <p class="text-2xl font-bold theme-text-primary">
              {{ getTotalReturns() | number: "1.0-0" }}€
            </p>
            <p class="text-xs theme-text-muted mt-1">
              {{ translationService.translate("statistics.fromInvestments") }}
            </p>
          </div>
          ...
          <div class="theme-bg-muted rounded-lg p-4 theme-border border">
            <div class="flex items-center justify-between mb-2">
              <span class="text-sm font-medium theme-text-muted">{{
                translationService.translate("statistics.averageROI")
              }}</span>
            ...
            <p class="text-xs theme-text-muted mt-1">
              {{ translationService.translate("statistics.annual") }}
            </p>
          </div>
          ...
          <div class="theme-bg-muted rounded-lg p-4 theme-border border">
            <div class="flex items-center justify-between mb-2">
              <span class="text-sm font-medium theme-text-muted">{{
                translationService.translate("statistics.turnsPlayed")
              }}</span>
            ...
            <p class="text-xs theme-text-muted mt-1">
              {{ game.age }} {{ translationService.translate("game.years") }}
            </p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <p-card
            [header]="
              translationService.translate('statistics.netWorthEvolution')
            "
            class="theme-shadow-md"
          >
            <p-chart
              type="line"
          ...
          <p-card
            [header]="
              translationService.translate('statistics.incomeVsExpenses')
            "
            class="theme-shadow-md"
          >
            <p-chart
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <p-card
            [header]="
              translationService.translate('statistics.portfolioDistribution')
            "
            class="theme-shadow-md"
          >
            <p-chart
          ...
          <!-- Cash Flow Trend -->
          <p-card
            [header]="translationService.translate('statistics.cashFlowTrend')"
            class="theme-shadow-md"
          >
            <p-chart
              type="bar"
        ...
        <p-card
          [header]="
            translationService.translate('statistics.investmentPerformance')
          "
          class="theme-shadow-md"
        >
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b theme-border">
                  <th class="text-left p-2 theme-text-primary">
                    {{ translationService.translate("statistics.investment") }}
                  </th>
                  <th class="text-right p-2 theme-text-primary">
                    {{ translationService.translate("statistics.amount") }}
                  </th>
                  <th class="text-right p-2 theme-text-primary">
                    {{
                      translationService.translate("statistics.annualReturn")
                    }}
                  </th>
                  <th class="text-right p-2 theme-text-primary">
                    {{ translationService.translate("statistics.roi") }}
                  </th>
                  <th class="text-right p-2 theme-text-primary">
                    {{
                      translationService.translate("statistics.paybackYears")
                    }}
                  </th>
                </tr>
              </thead>
              <tbody>
                <tr
                  *ngFor="let investment of game.investments"
                  class="border-b theme-border hover:theme-bg-muted"
                >
                  <td class="p-2 theme-text-primary">{{ investment.name }}</td>
                  <td class="text-right p-2 theme-text-primary">
                    {{ investment.amount | number: "1.0-0" }}€
                  </td>
                  <td
                    class="text-right p-2 text-success-600 dark:text-success-400"
                  >
                    {{ investment.income | number: "1.0-0" }}€
                  </td>
                  <td class="text-right p-2 theme-text-primary font-semibold">
                    {{
                      (investment.income / investment.amount) * 100
                        | number: "1.1-1"
                    }}%
                  </td>
                  <td class="text-right p-2 theme-text-muted">
                    {{
                      investment.amount / investment.income | number: "1.1-1"
                    }}
                  </td>
                </tr>
              </tbody>
              <tfoot *ngIf="game.investments.length > 0">
                <tr class="font-semibold theme-bg-muted">
                  <td class="p-2 theme-text-primary">
                    {{ translationService.translate("statistics.total") }}
                  </td>
                  <td class="text-right p-2 theme-text-primary">
                    {{ getTotalInvested() | number: "1.0-0" }}€
                  </td>
                  <td
                    class="text-right p-2 text-success-600 dark:text-success-400"
                  >
            ...
            <div
              *ngIf="game.investments.length === 0"
              class="text-center py-8 theme-text-muted"
            >
...
export class GameStatisticsComponent implements OnInit, OnDestroy {
  ...
  updateCharts() {
    ...
    const labels = history.map(
      (h) =>
        `${this.translationService.translate("game.turn")} ${h.turnNumber}`,
    );
    ...
    this.game.investments.forEach((inv) => {
      typeDistribution[inv.type] =
        (typeDistribution[inv.type] || 0) + inv.amount;
    });
    ...
    const cashFlowLabels = recentHistory.map(
      (h) =>
        `${this.translationService.translate("game.turn")} ${h.turnNumber}`,
    );
      ...
      datasets: [
        {
          ...
          data: cashFlowData,
          backgroundColor: cashFlowData.map((v) =>
            v >= 0 ? "rgba(34, 197, 94, 0.8)" : "rgba(239, 68, 68, 0.8)",
          ),
          borderColor: cashFlowData.map((v) =>
            v >= 0 ? "#22c55e" : "#ef4444",
          ),
          borderWidth: 1,
        },
