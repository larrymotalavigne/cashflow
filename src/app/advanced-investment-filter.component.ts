      ...
      <ng-template pTemplate="header">
        <div class="flex justify-content-between align-items-center p-3">
          <h4 class="m-0 theme-text-card flex items-center">
            <i
              class="pi pi-filter mr-2 text-primary-600 dark:text-primary-400"
            ></i>
            Filtres Avancés
          </h4>
      ...
      <ng-template pTemplate="content">
        ...
        <div class="flex flex-wrap gap-3 mb-4">
          <p-button
            ...
            [styleClass]="
              currentFilter.showAffordableOnly
          ...
          <div class="advanced-filters animate-slide-down">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                >
                  ROI ({{ currentFilter.roiRange[0] }}% -
                  {{ currentFilter.roiRange[1] }}%)
                </label>
                <p-slider
              ...
              <div>
                <label
                  class="block text-sm font-medium theme-text-primary mb-2"
                >
                  Prix ({{ formatCurrency(currentFilter.priceRange[0]) }} -
                  {{ formatCurrency(currentFilter.priceRange[1]) }})
                </label>
                <p-slider
              ...
              <div>
                ...
                >
                  Revenu mensuel ({{
                    formatCurrency(currentFilter.incomeRange[0])
                  }}
                  - {{ formatCurrency(currentFilter.incomeRange[1]) }})
                </label>
                <p-slider
...
export class AdvancedInvestmentFilterComponent implements OnInit {
  ...
  private applyFilters() {
    ...
    filtered = filtered.filter((inv) => {
      ...
      return (
        roi >= this.currentFilter.roiRange[0] &&
        roi <= this.currentFilter.roiRange[1]
      );
    });
    ...
    this.affordableCount = this.investments.filter(
      (inv) => inv.amount <= this.userCash,
    ).length;
  ...
  toggleSortDirection() {
    this.currentFilter.sortDirection =
      this.currentFilter.sortDirection === "asc" ? "desc" : "asc";
  ...
  toggleAffordableFilter() {
    this.currentFilter.showAffordableOnly =
      !this.currentFilter.showAffordableOnly;
    this.onFilterChange();
  }
