    ...
    <p-card class="theme-bg-card theme-shadow-lg">
      ...
      <ng-template pTemplate="content">
              ...
              <div class="space-y-2">
                @for (series of dataSeriesOptions; track series) {
                  <div class="flex align-items-center">
                    ...
                    <label
                      [for]="series.key"
                      class="ml-2 text-sm theme-text-primary cursor-pointer"
                    >
                      {{ series.label }}
                    </label>
                  </div>
        ...
        @if (financialGoals.length > 0) {
          <div class="mb-4 p-3 theme-bg-muted rounded-lg">
            <h4 class="theme-text-primary mb-3 flex items-center">
              <i
                class="pi pi-flag mr-2 text-primary-600 dark:text-primary-400"
              ></i>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              @for (goal of financialGoals; track goal) {
                <div class="p-3 theme-bg-card rounded-lg theme-shadow-sm">
                  ...
                  >
                    <span class="font-medium theme-text-card">{{
                      goal.name
                    }}</span>
                    <span
                      class="text-sm"
                  ...
                  >
                    <div
                      ...
                      [ngClass]="
                        goal.achieved ? 'bg-success-500' : 'bg-primary-500'
                      "
                      [style.width.%]="
                        Math.min(getProgressPercentage(goal), 100)
                      "
                    ></div>
                  </div>
        ...
        <div class="mt-4 p-3 theme-bg-muted rounded-lg">
          <h4 class="theme-text-primary mb-3 flex items-center">
            <i
              class="pi pi-chart-bar mr-2 text-primary-600 dark:text-primary-400"
            ></i>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="text-center">
              ...
              >
                {{ formatCurrency(getTotalPassiveIncome()) }}
              </div>
...
export class ProgressChartComponent implements OnInit, OnDestroy {
      ...
      switch (series.key) {
        ...
        case "netWorth":
          data_points = data.map((turn) => {
            ...
            const investmentValue = this.getInvestmentValueAtTurn(
              turn.turnNumber,
            );
        ...
        case "investments":
          data_points = data.map((turn) =>
            this.getInvestmentValueAtTurn(turn.turnNumber),
          );
          break;
      }
  ...
  private getInvestmentValueAtTurn(turnNumber: number): number {
    ...
    const currentInvestments = this.game.investments.reduce(
      (sum, inv) => sum + inv.amount,
      0,
    );
    return (
      currentInvestments *
      (turnNumber / Math.max(this.game.turnHistory.length, 1))
    );
  }
  ...
  updateFinancialGoals() {
    ...
    this.financialGoals[0].achieved =
      this.game.passiveIncome >= this.game.expenses;

    // Update net worth goal
    this.financialGoals[1].currentValue = this.getNetWorth();
    this.financialGoals[1].achieved =
      this.getNetWorth() >= this.financialGoals[1].targetValue;

    // Update passive income goal
    this.financialGoals[2].currentValue = this.game.passiveIncome;
    this.financialGoals[2].achieved =
      this.game.passiveIncome >= this.financialGoals[2].targetValue;
  }
  ...
  getNetWorth(): number {
    const investmentValue = this.game.investments.reduce(
      (sum, inv) => sum + inv.amount,
      0,
    );
  ...
  getROI(): number {
    const totalInvested = this.game.investments.reduce(
      (sum, inv) => sum + inv.amount,
      0,
    );
  ...
  exportChart() {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute(
        "download",
        `cashflow-evolution-${new Date().getTime()}.csv`,
      );
