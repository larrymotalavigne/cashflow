    ...
    <p-card class="theme-bg-card theme-shadow-lg">
      <ng-template pTemplate="header">
        <div class="flex justify-content-between align-items-center p-3">
          <h3 class="m-0 theme-text-card">
            🎯 Gestion de Portfolio (Glisser-Déposer)
          </h3>
        ...
        @if (investmentGroups.length === 0) {
          <div class="text-center p-6">
            ...
            <p class="theme-text-muted">
              Achetez des investissements pour commencer à gérer votre
              portfolio.
            </p>
          </div>
        }
          ...
          <div class="portfolio-manager">
            ...
            <div class="mb-4 p-3 theme-bg-muted rounded-lg">
              <h4 class="theme-text-primary mb-3 flex items-center">
                <i
                  class="pi pi-chart-pie mr-2 text-primary-600 dark:text-primary-400"
                ></i>
            ...
            <!-- Drag & Drop Groups -->
            <div
              class="investment-groups grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4"
            >
              @for (group of investmentGroups; track group) {
                ...
                >
                  <div
                    class="group-header p-3 rounded-t-lg"
                    [style.background-color]="group.color + '20'"
                  >
                    <div class="flex justify-between items-center">
                      <div class="flex items-center gap-2">
                        <span class="text-lg">{{ group.icon }}</span>
                        <h4 class="theme-text-primary font-semibold">
                          {{ group.name }}
                        </h4>
                        <span
                          class="text-xs theme-bg-card px-2 py-1 rounded-full theme-text-muted"
                        >
                  ...
                  >
                    @for (
                      investment of group.investments;
                      track trackByInvestmentName($index, investment)
                    ) {
                      <div
                      ...
                      >
                        <div class="flex justify-between items-start">
                          <div class="flex-1 min-w-0">
                            <h5
                              class="theme-text-primary font-semibold text-sm truncate mb-1"
                            >
                              {{ investment.name }}
                            </h5>
                            <div
                            ...
                            <div class="mt-1">
                              <span
                                ...
                                [ngClass]="getROIClass(investment)"
                              >
                                ROI:
                                {{ getInvestmentROI(investment).toFixed(1) }}%
                              </span>
                            </div>
                          </div>
                    ...
                    @if (group.investments.length === 0) {
                      <div class="text-center py-6 theme-text-muted">
            ...
            <div class="mt-6 p-4 theme-bg-muted rounded-lg">
              <h4 class="theme-text-primary mb-2 flex items-center">
                <i
                  class="pi pi-info-circle mr-2 text-primary-600 dark:text-primary-400"
                ></i>
                Instructions
              </h4>
              <ul class="text-sm theme-text-muted space-y-1">
                <li>
                  • <strong>Glisser-déposer</strong> : Déplacez les
                  investissements entre les catégories
                </li>
                <li>
                  • <strong>Réorganiser</strong> : Changez l'ordre au sein d'une
                  même catégorie
                </li>
                <li>
                  • <strong>Vendre</strong> : Cliquez sur ✕ pour vendre un
                  investissement
                </li>
                <li>
                  • <strong>Tri automatique</strong> : Utilisez le bouton de tri
                  pour organiser par type
                </li>
              </ul>
            </div>
          </div>
...
export class DragDropPortfolioComponent implements OnInit {
  ...
  private getTypeFromGroupId(
    groupId: string,
  ): "capital" | "real_estate" | "business" | "crypto" | "fund" {
    const groupMap: {
      [key: string]: "capital" | "real_estate" | "business" | "crypto" | "fund";
    } = {
  ...
  onDrop(event: DragEvent, targetGroupId: string) {
    ...
    const sourceGroup = this.investmentGroups.find(
      (g) => g.id === this.sourceGroupId,
    );
    const targetGroup = this.investmentGroups.find(
    ...
    const investmentIndex = sourceGroup.investments.findIndex(
      (inv) => inv.name === this.draggedInvestment!.name,
    );
  ...
  private updateGroupTotals() {
    this.investmentGroups.forEach((group) => {
      group.totalValue = group.investments.reduce(
        (sum, inv) => sum + inv.amount,
        0,
      );
      group.totalIncome = group.investments.reduce(
  ...
  async sellInvestment(investment: Investment) {
    ...
    if (result.confirmed) {
      ...
      const index = this.gameService.investments.findIndex(
        (inv) => inv.name === investment.name,
      );
      if (index > -1) {
  ...
  autoSortByType() {
    ...
    this.toastService.info(
      "Portfolio trié",
      "Les investissements ont été organisés par type",
    );
  }

  getTotalPortfolioValue(): number {
    return this.investmentGroups.reduce(
  ...
  getTotalMonthlyIncome(): number {
    ...
    return (
      this.investmentGroups.reduce((sum, group) => sum + group.totalIncome, 0) /
      12
    );
  }
  ...
  getInvestmentROI(investment: Investment): number {
    return investment.amount > 0
      ? ((investment.income * 12) / investment.amount) * 100
      : 0;
  }
