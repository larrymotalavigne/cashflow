...
export class GameService {
  ...
  loadGameState(): boolean {
    ...
    if (savedState) {
      try {
        ...
        this.selectedOpportunitiesPerYear =
          gameState.selectedOpportunitiesPerYear || {};
        ...
        this.retirementAccounts =
          gameState.retirementAccounts || this.initializeRetirementAccounts();
        this.retirementPlan =
          gameState.retirementPlan || this.createInitialRetirementPlan();
        return true;
      } catch (error) {
  ...
  nextTurn() {
    ...
    if (currentCycle && currentCycle.effects.inflationRate) {
      ...
      console.log(
        `Inflation applied: ${(inflationRate * 100).toFixed(1)}%, cumulative: ${((this.cumulativeInflation - 1) * 100).toFixed(1)}%`,
      );
      console.log(
        `Income adjusted to: $${this.income}, Expenses adjusted to: $${this.expenses}`,
      );
    }
    ...
    this.investments.forEach((investment) => {
      let baseReturn = this.configService.applyDifficultyToInvestmentReturn(
        investment.income,
      );
    ...
    this.cash +=
      this.income + this.passiveIncome - this.expenses - liabilityPayments; // Subtract liability payments
    console.log(
      `Next turn - ${this.cash} - ${this.income + this.passiveIncome - this.expenses - liabilityPayments}`,
    );
    ...
    this.randomEvents.forEach((event) => {
      if (event.effect?.type === "cash") {
        const modifiedAmount = this.configService.applyEconomicCycleToEvent(
          event.effect.amount,
        );
    ...
    const taxCalculation = this.calculateTaxes(
      this.income,
      capitalGains,
      dividendIncome,
    );
    ...
    console.log(
      `Taxes calculated: Income Tax: $${taxCalculation.incomeTax.toFixed(2)}, Capital Gains Tax: $${taxCalculation.capitalGainsTax.toFixed(2)}, Total Tax: $${taxCalculation.totalTax.toFixed(2)}`,
    );
    ...
    if (this.age >= 50) {
      ...
      const account401k = this.retirementAccounts.find(
        (acc) => acc.type === "401k",
      );
      if (account401k) {
  ...
  showOpportunities() {
    ...
    const currentYearSelected =
      this.selectedOpportunitiesPerYear[this.age] || [];

    // Get weighted random events, filtering out already selected ones
    this.randomEvents = this.getWeightedRandomEvents().filter(
      (event) => !currentYearSelected.includes(event.message),
    );

    // Get random investments, filtering out already selected ones
    const availableInvestments = this.configService.investments.filter(
      (investment) => !currentYearSelected.includes(investment.name),
    );
    const shuffledInvestments = [...availableInvestments].sort(
      () => 0.5 - Math.random(),
    );
    const numInvestments =
      Math.floor(
        Math.random() *
          (this.maxInvestmentOpportunities -
            this.minInvestmentOpportunities +
            1),
      ) + this.minInvestmentOpportunities;
  ...
  private getWeightedRandomEvents(): GameEvent[] {
    ...
    const numEvents =
      Math.floor(
        Math.random() * (this.maxRandomEvents - this.minRandomEvents + 1),
      ) + this.minRandomEvents;
      ...
      if (event.effect) {
        ...
        if (event.effect.type === "cash") {
          ...
          if (event.effect.amount > 0) {
            if (event.effect.amount > 5000)
              weight = 0.2; // Very rare
            else if (event.effect.amount > 2000)
              weight = 0.5; // Rare
            else if (event.effect.amount > 1000)
              weight = 0.8; // Uncommon
  ...
  checkWinCondition() {
    if (this.passiveIncome >= this.expenses) {
      this.eventMessage = this.translationService.translate(
        "game.victoryMessage",
      );
      this.eventVisible = true;
  ...
  calculateLiabilityPayments(): number {
    // New method
    return this.investments.reduce(
      (sum, l) => sum + (l.yearlyPayment || 0) / 12,
      0,
    );
  }
  ...
  calculateRiskScore(investment: Investment): number {
    ...
    switch (investment.type) {
      case "capital":
        baseRisk = 2;
        break; // Stocks are moderate risk
      case "real_estate":
        baseRisk = 3;
        break; // Real estate moderate-low risk
      case "business":
        baseRisk = 6;
        break; // Business investments are higher risk
      case "crypto":
        baseRisk = 9;
        break; // Crypto is very high risk
      case "fund":
        baseRisk = 2;
        break; // Funds are moderate risk
    }
  ...
  private getSectorProperties(sector: InvestmentSector): {
    ...
    switch (sector) {
      case "technology":
        return {
          sectorRiskMultiplier: 1.5,
          sectorReturnMultiplier: 1.3,
          sectorVolatility: 0.2,
        };
      case "healthcare":
        return {
          sectorRiskMultiplier: 1.2,
          sectorReturnMultiplier: 1.15,
          sectorVolatility: 0.1,
        };
      case "real_estate":
        return {
          sectorRiskMultiplier: 0.8,
          sectorReturnMultiplier: 1.1,
          sectorVolatility: 0.05,
        };
      case "finance":
        return {
          sectorRiskMultiplier: 1.1,
          sectorReturnMultiplier: 1.05,
          sectorVolatility: 0.15,
        };
      case "energy":
        return {
          sectorRiskMultiplier: 1.4,
          sectorReturnMultiplier: 1.2,
          sectorVolatility: 0.25,
        };
      case "consumer_goods":
        return {
          sectorRiskMultiplier: 0.9,
          sectorReturnMultiplier: 1.0,
          sectorVolatility: 0.08,
        };
      case "utilities":
        return {
          sectorRiskMultiplier: 0.6,
          sectorReturnMultiplier: 0.9,
          sectorVolatility: 0.03,
        };
      case "telecommunications":
        return {
          sectorRiskMultiplier: 1.0,
          sectorReturnMultiplier: 1.0,
          sectorVolatility: 0.12,
        };
      case "industrials":
        return {
          sectorRiskMultiplier: 1.1,
          sectorReturnMultiplier: 1.08,
          sectorVolatility: 0.15,
        };
      case "materials":
        return {
          sectorRiskMultiplier: 1.3,
          sectorReturnMultiplier: 1.12,
  ...
  initializeInvestmentRisk(investment: Investment): Investment {
    ...
    const totalVolatility =
      volatility + (sectorProperties.sectorVolatility || 0);
  ...
  startGame(
    ...
    const expenses = this.configService.applyDifficultyToExpenses(
      selectedJob.expenses,
    );
    const cash =
      this.configService.applyDifficultyToStartingCash(startingMoney);
  ...
  calculateTaxes(
    ...
    const applicableBracket =
      brackets.find(
        (b) =>
          grossIncome >= b.minIncome &&
          (!b.maxIncome || grossIncome <= b.maxIncome),
    ...
    const effectiveTaxRate =
      grossIncome + capitalGains + dividendIncome > 0
  ...
  contributeToRetirement(
    ...
    const account = this.retirementAccounts.find(
      (acc) => acc.type === accountType,
    );
    if (!account) return false;

    const remainingContributionRoom =
      account.maxContribution - account.contributions;
    ...
    if (actualContribution > 0) {
      ...
      if (accountType === "401k" && account.employerMatch) {
        const employerMatch = Math.min(
          actualContribution * account.employerMatch,
          this.income * account.employerMatch,
        );
  ...
  updateRetirementProjections(): void {
    ...
    const yearsToRetirement = Math.max(
      0,
      this.retirementPlan.targetRetirementAge - this.age,
    );
    const totalContributions = this.retirementAccounts.reduce(
      (sum, acc) => sum + acc.contributions,
      0,
    );
    const currentBalance = this.retirementAccounts.reduce(
    ...
    const requiredBalance =
      (this.retirementPlan.monthlyRetirementGoal * 12) / 0.04;
