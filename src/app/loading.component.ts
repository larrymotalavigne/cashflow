...
@Component({
  ...
  template: `
    <div
      class="fixed inset-0 bg-black/20 dark:bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center animate-fade-in"
    >
      <div
        class="theme-bg-card theme-shadow-xl rounded-2xl p-8 max-w-sm mx-4 text-center animate-slide-up"
      >
        <div class="flex justify-center mb-4">
          <app-loading [variant]="variant" [size]="size"></app-loading>
        </div>
        @if (title) {
          <h3 class="text-lg font-semibold theme-text-card mb-2">
            {{ title }}
          </h3>
        }
        @if (message) {
...
@Component({
  ...
  template: `
    <div
      class="financial-counter font-mono text-right"
      [ngClass]="getValueClasses()"
    >
      <span class="currency text-sm opacity-75">{{ currency }}</span>
      <span
        class="value text-xl font-bold ml-1 tabular-nums"
        [attr.data-value]="animatedValue"
      >
        {{ displayValue }}
      </span>
    ...
    `
      ...
      .value-increased {
        animation:
          ...
          pulse 0.3s ease-out 0.1s;
      }

      .value-decreased {
        animation:
          countDown 0.6s ease-out,
          pulse 0.3s ease-out 0.1s;
      }
