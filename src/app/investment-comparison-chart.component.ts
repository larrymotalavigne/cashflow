      ...
      <ng-template pTemplate="content">
        @if (investmentPerformance.length === 0) {
          <div class="text-center p-6">
            ...
            <h4 class="theme-text-primary mb-2">
              Aucun investissement à comparer
            </h4>
            <p class="theme-text-muted">
              Achetez des investissements pour voir leur performance
              comparative.
            </p>
          </div>
        }
          ...
          <div>
            ...
            <div class="mb-4">
              <h4 class="theme-text-primary mb-3 flex items-center">
                <i
                  class="pi pi-chart-line mr-2 text-primary-600 dark:text-primary-400"
                ></i>
                Performance par Investissement
              </h4>
              <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
                @for (
                  investment of investmentPerformance;
                  track trackByName($index, investment)
                ) {
                  <div
                    class="p-3 theme-bg-card rounded-lg theme-shadow-sm border-l-4"
                    [style.border-left-color]="
                      getInvestmentColor(investment.type)
                    "
                  >
                    <div
                      class="flex justify-content-between align-items-start mb-2"
                    >
                      <h5 class="theme-text-card mb-1">
                        {{ investment.name }}
                      </h5>
                      <span
                        class="text-xs theme-bg-muted px-2 py-1 rounded-full"
                    ...
                    <div class="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        ...
                        <span class="font-semibold theme-text-card ml-1">{{
                      ...
                      <div>
                        ...
                        <span
                          ...
                          [ngClass]="getROIClass(investment.roi)"
                        >
                      ...
                      <div>
                        ...
                        <span class="font-semibold theme-text-card ml-1"
                          >{{ investment.paybackPeriod }} mois</span
                        >
                      </div>
                    </div>
                  </div>
            ...
            <div class="mb-4">
              <h4 class="theme-text-primary mb-3 flex items-center">
                <i
                  class="pi pi-objects-column mr-2 text-primary-600 dark:text-primary-400"
                ></i>
                Analyse par Type d'Investissement
              </h4>
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @for (typeStats of investmentTypeStats; track typeStats) {
                  <div class="p-4 theme-bg-muted rounded-lg text-center">
                    <div class="text-2xl mb-2">
                      {{ getTypeIcon(typeStats.type) }}
                    </div>
                    <h5 class="theme-text-primary mb-2">
                      {{ typeStats.type }}
                    </h5>
                    <div class="space-y-1 text-sm">
                      <div>
                        <span class="theme-text-muted">Nombre:</span>
                        <span class="font-semibold theme-text-primary ml-1">{{
                          typeStats.count
                        }}</span>
                      </div>
                      <div>
                        ...
                        <span class="font-semibold theme-text-primary ml-1">{{
                          formatCurrency(typeStats.totalInvested)
                        }}</span>
                      </div>
                      <div>
                        ...
                        <span
                          class="font-semibold ml-1"
                          [ngClass]="getROIClass(typeStats.avgROI)"
                        >
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="p-4 theme-bg-muted rounded-lg">
                <h5 class="theme-text-primary mb-3 flex items-center">
                  <i
                    class="pi pi-crown mr-2 text-success-600 dark:text-success-400"
                  ></i>
                  Meilleur Investissement
                </h5>
                @if (bestPerformer) {
                  <div class="p-3 theme-bg-card rounded-lg">
                    <h6 class="theme-text-card mb-2">
                      {{ bestPerformer.name }}
                    </h6>
              ...
              <div class="p-4 theme-bg-muted rounded-lg">
                <h5 class="theme-text-primary mb-3 flex items-center">
                  <i
                    class="pi pi-exclamation-triangle mr-2 text-warning-600 dark:text-warning-400"
                  ></i>
                  Moins Performant
                </h5>
                @if (worstPerformer) {
                    ...
                    <div class="text-sm space-y-1">
                      <div class="flex justify-content-between">
                        ...
                        <span
                          ...
                          >{{ worstPerformer.roi.toFixed(1) }}%</span
                        >
                      </div>
                      <div class="flex justify-content-between">
                        <span class="theme-text-muted">Retour mensuel:</span>
                        <span class="font-semibold theme-text-card">{{
                          formatCurrency(worstPerformer.monthlyReturn)
                        }}</span>
                      </div>
                    </div>
                  </div>
...
export class InvestmentComparisonChartComponent implements OnInit, OnDestroy {
          ...
          callbacks: {
            label: (context: any) => {
              ...
              if (this.selectedComparisonType === "roi") {
                ...
                this.selectedComparisonType === "monthlyReturn" ||
                this.selectedComparisonType === "totalInvested"
              ) {
  ...
  updateData() {
    this.investmentPerformance = this.game.investments.map((investment) => {
      ...
      const paybackPeriod =
        investment.income > 0
          ? Math.ceil(investment.amount / investment.income)
          : 0;
  ...
  private calculateTypeStats() {
    const typeMap = new Map<
      string,
      { count: number; totalInvested: number; totalROI: number }
    >();
    ...
    this.investmentTypeStats = Array.from(typeMap.entries()).map(
      ([type, stats]) => ({
        type,
        count: stats.count,
  ...
  updateChart() {
    ...
    const colors = this.investmentPerformance.map((inv) =>
      this.getInvestmentColor(inv.type),
    );
  ...
  exportComparison() {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute(
        "download",
        `investment-comparison-${new Date().getTime()}.csv`,
      );
