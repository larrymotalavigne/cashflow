...
@Component({
  ...
  template: `
    <div
      class="investment-card theme-bg-card theme-shadow-lg hover:theme-shadow-2xl transition-all duration-300 rounded-2xl border-2 theme-border overflow-hidden group hover:scale-[1.03] cursor-pointer relative"
    ...
    >
      ...
      <!-- Header Section -->
      <div
        class="card-header p-4 pb-3 border-b theme-border bg-gradient-to-r from-transparent to-primary-50/30 dark:to-primary-900/20"
      >
      ...
      <div class="card-metrics p-4 py-3">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="metric-item">
            <div class="flex items-center gap-1 mb-1">
              ...
              <span
                class="text-xs font-medium theme-text-muted uppercase tracking-wide"
                >{{ translationService.translate("investments.price") }}</span
              >
            </div>
          ...
          <div class="metric-item">
            <div class="flex items-center gap-1 mb-1">
              ...
              <span
                class="text-xs font-medium theme-text-muted uppercase tracking-wide"
                >{{
                  translationService.translate("investments.yearlyLabel")
                }}</span
              >
            </div>
            <app-financial-counter
        ...
        <div class="mt-3 flex items-center justify-between">
          <div class="flex items-center gap-1">
            ...
            <span class="text-xs theme-text-muted"
              >{{
                translationService.translate("investments.paybackPeriod")
              }}:</span
            >
          </div>
          <span class="text-sm font-semibold theme-text-card"
            >{{ getPaybackPeriod() }}
            {{ translationService.translate("investments.months") }}</span
          >
        </div>
        ...
        <div class="mt-3">
          <div class="flex justify-between items-center mb-1">
            <span class="text-xs theme-text-muted">{{
              translationService.translate("investments.performance")
            }}</span>
            <span class="text-xs font-medium" [ngClass]="getROITextClass()">{{
      ...
      <!-- Actions Section -->
      <div
        class="card-actions p-4 pt-3 bg-gradient-to-r from-neutral-50/50 via-primary-50/20 to-secondary-50/30 dark:from-neutral-800/50 dark:via-primary-900/20 dark:to-secondary-900/30"
      >
        <div
          class="grid grid-cols-1 gap-2"
          [ngClass]="comparisonMode ? 'grid-cols-1' : 'grid-cols-2'"
        >
          <!-- Primary Actions -->
          @if (!comparisonMode) {
            <div class="flex flex-col gap-2">
              ...
              <p-button
                [label]="
                  canAfford
                    ? translationService.translate('investments.actions.buy')
                    : translationService.translate(
                        'investments.actions.insufficientFunds'
                      )
                "
                [icon]="
                  canAfford
                ...
                [styleClass]="
                  'w-full p-button-sm ' +
                  (canAfford ? 'p-button-success' : 'p-button-warning')
                "
                [pTooltip]="
                  canAfford
                    ? translationService.translate(
                        'investments.actions.buyWithCash'
                      )
                    : translationService.translate(
                        'investments.actions.insufficientFunds'
                      )
              ...
              <p-button
                [label]="
                  translationService.translate('investments.actions.loan') +
                  ' (' +
                  configService.loanRate * 100 +
                  '%)'
                "
                ...
                [pTooltip]="
                  translationService.translate(
                    'investments.actions.loanTooltip'
                  ) +
                  ': ' +
                  configService.loanRate * 100 +
                  '%)'
          ...
          <div class="flex flex-col gap-2">
            <p-button
              [label]="
                translationService.translate('investments.actions.reject')
              "
              icon="pi pi-times"
              (click)="onReject()"
              ...
              [pTooltip]="
                translationService.translate(
                  'investments.actions.rejectTooltip'
                )
              "
            >
            </p-button>
            ...
            @if (!comparisonMode) {
              <p-button
                [label]="
                  translationService.translate('investments.actions.compare')
                "
                ...
                [pTooltip]="
                  translationService.translate(
                    'investments.actions.compareTooltip'
                  )
                "
              >
              </p-button>
      ...
      <div
        class="absolute inset-0 bg-primary-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none rounded-xl"
      ></div>
    </div>
  `,
  styles: [
...
export class EnhancedInvestmentCardComponent {
  ...
  getRiskLevel(): string {
      ...
      switch (this.investment.riskCategory) {
        case "very_high":
          return this.translationService.translate(
            "investments.riskLevels.veryHigh",
          );
        case "high":
          return this.translationService.translate(
            "investments.riskLevels.high",
          );
        case "medium":
          return this.translationService.translate(
            "investments.riskLevels.medium",
          );
        case "low":
          return this.translationService.translate(
            "investments.riskLevels.low",
          );
        default:
          return this.translationService.translate(
            "investments.riskLevels.medium",
    ...
    if (roi > 20)
      return this.translationService.translate("investments.riskLevels.high");
    if (roi > 12)
      return this.translationService.translate("investments.riskLevels.medium");
  ...
  getRiskIcon(): string {
    ...
    if (this.investment.riskCategory) {
      switch (this.investment.riskCategory) {
        case "very_high":
          return "pi-exclamation-triangle text-error-600";
        case "high":
          return "pi-exclamation-triangle text-error-500";
        case "medium":
          return "pi-info-circle text-warning-500";
        case "low":
  ...
  getRiskTooltip(): string {
    ...
    const roi = this.getROIValue();
    if (roi > 20)
      return "Investissement à haut risque - rendements élevés mais volatils";
    if (roi > 12)
      return "Investissement à risque modéré - équilibre risque/rendement";
  ...
  getPerformanceRating(): string {
    ...
    if (roi > 15)
      return this.translationService.translate(
        "investments.performanceRatings.excellent",
      );
    if (roi > 8)
      return this.translationService.translate(
        "investments.performanceRatings.good",
      );
    if (roi > 0)
      return this.translationService.translate(
        "investments.performanceRatings.fair",
      );
    return this.translationService.translate(
      "investments.performanceRatings.poor",
    );
  }
  ...
  onBuy(): void {
    if (this.canAfford) {
      ...
      const card = document.querySelector(
        `[data-investment="${this.investment.name}"]`,
      );
      if (card) {
