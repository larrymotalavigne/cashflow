...
@Component({
  ...
  template: `
    ...
    >
      <div
        class="theme-bg-card rounded-lg p-6 theme-shadow-xl flex flex-col items-center gap-4"
      >
        <i class="pi pi-spin pi-spinner text-4xl text-primary-500"></i>
        <span class="theme-text-primary font-semibold">{{
          translationService.translate("common.loading")
        }}</span>
      </div>
    </div>

    <p-toolbar
      class="theme-bg-card theme-shadow-md border-b-2 border-primary-200 dark:border-primary-800 relative overflow-hidden"
    >
      <!-- Animated gradient border -->
      <div
        class="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-primary-500 via-secondary-500 to-accent-500"
      ></div>
      ...
      <div class="flex align-items-center justify-between w-full relative">
        ...
        <div class="flex align-items-center">
          ...
          >
            <i
              class="pi pi-arrow-left text-lg sm:text-xl theme-text-primary"
            ></i>
          </button>
        </div>
        ...
        <div
          class="absolute left-1/2 transform -translate-x-1/2 flex align-items-center gap-3"
        >
          <div class="relative">
            <div
              class="absolute inset-0 bg-gradient-to-r from-primary-500 to-secondary-500 rounded-full blur-md opacity-50"
            ></div>
            <div
              class="relative bg-gradient-to-br from-primary-500 to-secondary-500 p-2 rounded-full"
            >
              <svg
          ...
          <span
            class="text-lg sm:text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary-600 to-secondary-600 dark:from-primary-400 dark:to-secondary-400"
            >{{ translationService.translate("game.dashboard") }}</span
          >
        </div>
    ...
    <div
      class="w-full max-w-6xl mx-auto px-3 sm:px-4 py-4 min-h-screen theme-bg-primary bg-gradient-to-br from-primary-50/50 to-secondary-50/30 dark:from-neutral-900/50 dark:to-primary-900/30 
    ...
    >
      ...
      <app-random-event-dialog
        [visible]="this.game.eventVisible"
        [message]="this.game.eventMessage"
        [investmentOpportunities]="this.game.investmentOpportunities"
        [randomEvents]="this.game.randomEvents"
        (visibleChange)="this.game.eventVisible = $event"
      ></app-random-event-dialog>
    </div>
  `,
})
