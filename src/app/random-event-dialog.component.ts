...
@Component({
  ...
  imports: [
    ...
    InputTextModule,
    DividerModule,
    AdvancedInvestmentFilterComponent,
    InvestmentCarouselComponent,
  ],
  template: `
    <p-dialog
      #dialog
      ...
      [header]="getYearTitle()"
      [modal]="true"
      styleClass="w-75 theme-bg-card"
    >
      @if (randomEvents.length) {
        <div class="mb-4">
          <h3 class="theme-text-card mb-3">
            {{ translationService.translate("investments.events") }}
          </h3>
          @for (event of randomEvents; track event) {
            <div class="mb-2 theme-bg-muted p-3 rounded-lg theme-border border">
              ...
              @if (event.effect?.type) {
                <span class="ml-2 text-sm theme-text-muted"
                  >({{ event.effect.type }})</span
                >
              }
            </div>
          }
      ...
      <div>
        <h3 class="theme-text-card mb-3">
          {{ translationService.translate("investments.opportunities") }}
        </h3>
        ...
        @if (investmentOpportunities.length) {
          <div>
                ...
                [label]="
                  showAdvancedFilters
                    ? translationService.translate(
                        'investments.hideAdvancedFilters'
                      )
                    : translationService.translate(
                        'investments.showAdvancedFilters'
                      )
                "
                icon="pi pi-filter"
            ...
            @if (comparisonMode) {
              <div
                class="mb-3 p-3 theme-border border rounded-lg theme-bg-muted"
              >
                <div
                  class="flex justify-content-between align-items-center mb-3"
                >
                  <h4 class="m-0 theme-text-primary">
                    {{ translationService.translate("investments.comparison") }}
                  </h4>
                  <p-button
                    icon="pi pi-times"
                ...
                <div class="grid">
                  @for (investment of selectedInvestments; track investment) {
                    <div class="col-12 md:col-6 lg:col-4">
                      <div
                        class="p-3 theme-border border rounded-lg h-full theme-bg-card theme-shadow-sm"
                      >
                        <h5 class="theme-text-card">{{ investment.name }}</h5>
                        <div class="grid">
                          <div class="col-6 theme-text-muted">
                            {{
                              translationService.translate("investments.price")
                            }}:
                          </div>
                          <div class="col-6 font-bold theme-text-card">
                            {{ investment.amount }}€
                          </div>
                          <div class="col-6 theme-text-muted">
                            {{
                          ...
                          <div class="col-6 theme-text-muted">
                            {{
                              translationService.translate("investments.roi")
                            }}:
                          </div>
                          <div class="col-6 font-bold theme-text-card">
                            {{
                              (
                                (investment.income / investment.amount) *
                                100
                              ).toFixed(2)
                            }}%
                          </div>
                          <div class="col-6 theme-text-muted">
                            {{
                              translationService.translate("investments.type")
                            }}:
                          </div>
                          <div class="col-6 font-bold theme-text-card">
                            {{ investment.type }}
                        ...
                        <div class="mt-3">
                          <p-button
                            [label]="
                              translationService.translate('investments.remove')
                            "
                            (click)="removeFromComparison(investment)"
            ...
            <div class="flex justify-content-between align-items-center mb-3">
              @if (!comparisonMode) {
                <p-button
                  [label]="
                    translationService.translate(
                      'investments.compareInvestments'
                    )
                  "
          ...
          <p class="text-center text-sm theme-text-muted mt-2">
            {{ translationService.translate("investments.noOpportunities") }}
          </p>
        }
      </div>
      <ng-template #footer>
...
export class RandomEventDialogComponent {
  ...
  @Output() investmentAction = new EventEmitter<{
    investment: any;
    action: "buy" | "reject" | "loan";
  }>();
  ...
  updateTypeOptions() {
    ...
    const uniqueTypes = [
      ...new Set(this.configService.investments.map((inv) => inv.type)),
    ];
    this.typeOptions = uniqueTypes.map((type) => ({
      label: type,
      value: type,
    }));
  }
  ...
  deleteInvestment(investment: Investment) {
    this.investmentOpportunities = this.investmentOpportunities.filter(
      (i) => i !== investment,
    );
  ...
  onInvestmentSelectionChange(event: {
  ...
  toggleInvestmentSelection(investment: Investment) {
    const index = this.selectedInvestments.findIndex(
  ...
  removeFromComparison(investment: Investment) {
    this.selectedInvestments = this.selectedInvestments.filter(
      (inv) => inv.name !== investment.name,
    );
  }
